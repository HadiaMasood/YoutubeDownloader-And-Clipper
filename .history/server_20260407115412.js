...
app.get("/progress/:jobId", (req, res) => {
  ...
  console.log(`[GET /progress/${req.params.jobId}] Returning: progr ess=${job.progress}, status=${job.status}`);
...
function clipWithFFmpeg(job, url, startSec, endSec, format, filePath) {
  ...
  if (format === "mp3") {
    // ──MP3: Simple step-based progress ─────────────────────────────────────
    ...
    console.log(`[${job.jobId}] MP3: Downloading audio clip...`);
    job.progress = 10;  // advance to 10% when download starts

    const ytdlpProc = spawn(YTDLP, downloadArgs);
    let ytdlpLog = "";

    ytdlpProc.stdout.on("data", d => { ytdlpLog += d.toString(); });
    ytdlpProc.stderr.on("data", d => { ytdlpLog += d.toString(); });
    ...
    ytdlpProc.on("close", code => {
      if (code !== 0) {
        ...
        console.error(`[${job.jobId}] yt-dlp log:`, ytdlpLog.slice(-500));
        return;
      }

      // Audio download complete, advance to 50%
      job.progress = 50;
      console.log(`[${job.jobId}] MP3: Audio downloaded, starting FFmpeg conversion...`);
      ...
      // FFmpeg conversion starts, advance to 75%
      job.progress = 75;
      const ffProc = spawn(FFMPEG, ffArgs);
      let ffLog = "";

      ffProc.stderr.on("data", d => { ffLog += d.toString(); });
