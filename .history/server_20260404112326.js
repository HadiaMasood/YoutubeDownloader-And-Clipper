...
function clipWithFFmpeg(job, url, startSec, endSec, format, filePath) {
  ...
  if (format === "mp3") {
    // ── MP3: Download full audio (fast, small file) → ffmpeg clips it (instant) ──
    // This is MUCH faster than trying to clip during download
    const tempAudio = filePath + ".temp";
    ...
    console.log(`[${job.jobId}] Downloading full audio (small file, fast)...`);
    const ytProc = spawn(YTDLP, ytArgs);

    let ytLog = "";

    function parseYtProgress(txt) {
      const m = txt.match(/(\d+\.?\d*)%/);
      if (m) {
        const pct = Math.min(58, 5 + Math.round(parseFloat(m[1]) * 0.53));
    ...
    const hb1 = setInterval(() => {
      if (job.status === "clipping" && job.progress < 58) job.progress += 2;
    }, 600);
    ...
    ytProc.on("close", ytCode => {
      ...
      const possible = [tempAudio, tempAudio + ".m4a", tempAudio + ".webm", tempAudio + ".opus", tempAudio + ".mp3"];
      ...
      job.progress = 65;
      console.log(`[${job.jobId}] Audio downloaded: ${foundAudio}`);
      console.log(`[${job.jobId}] Clipping ${startHMS} → ${endHMS} and converting to MP3...`);

      // Clip and convert to MP3 (super fast from local file)
      const ffArgs = [
        "-ss", String(startSec),
        "-i", foundAudio,
        "-t", String(duration),
      ...
      ffProc.stderr.on("data", d => {
        ...
        if (m) {
          ...
          const pct = 65 + Math.round((elapsed / duration) * 33);
