...
app.post("/download", (req, res) => {
  try {
    fs.appendFileSync("C:/Users/WT/AppData/Local/Temp/test_debug.txt", "Handler called\n");
  } catch (e) {
  ...
  const debugMsg = `[${jobId}] hasClip=${hasClip}, startTime=${startTime}, endTime=${endTime}`;
  console.log(debugMsg);
  fs.appendFileSync(path.join(TMP, "debug.log"), debugMsg + "\n");
  if (hasClip) {
    const clipMsg = `[${jobId}] Calling clipWithFFmpeg`;
    fs.appendFileSync(path.join(TMP, "debug.log"), clipMsg + "\n");
    clipWithFFmpeg(job, url, Number(startTime), Number(endTime), format, filePath);
  } else {
    const fullMsg = `[${jobId}] Calling fullDownload`;
    fs.appendFileSync(path.join(TMP, "debug.log"), fullMsg + "\n");
...
function clipWithFFmpeg(job, url, startSec, endSec, format, filePath) {
  job.status = "clipping";
  job.progress = 10;  // Start at 10% instead of 5%
  const msg = `[${job.jobId}] clipWithFFmpeg: progress set to 10`;
  console.log(msg);
  fs.appendFileSync(path.join(TMP, "debug.log"), msg + "\n");
  ...
  if (format === "mp3") {
    ...
    let ytdlpLog = "";
    const ytdlpHeartbeat = setInterval(() => {
      if (job.status === "clipping" && job.progress < 35) job.progress += 1;
    }, 500);
