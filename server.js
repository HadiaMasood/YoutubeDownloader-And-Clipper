...
const FFMPEG_DIR = path.extname(FFMPEG) ? path.dirname(FFMPEG) : null;
...
function clipWithFFmpeg(job, url, startSec, endSec, format, filePath) {
  ...
  if (format === "mp3") {
    // ── MP3: pipe mode (yt-dlp → ffmpeg stdin, direct conversion) ───────────
    const ytArgs = [
      "--no-playlist",
    ...
    ytProc.stdout.on("end", () => { try { ffProc.stdin.end(); } catch (e) { } });
    ...
    ffProc.on("close", (code) => {
      clearInterval(heartbeat);
      if (code === 0 && fs.statSync(filePath).size > 1000) {
        job.status = "done";
        job.progress = 100;
        console.log(`[${job.jobId}] ✅ MP3 clip complete!`);
      } else {
        job.status = "error";
        job.error = "MP3 conversion failed or output was empty.";
